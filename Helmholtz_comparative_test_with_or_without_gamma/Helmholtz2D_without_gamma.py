...
@File    ：Helmholtz2D_without_gamma.py
@IDE     ：PyCharm 
@Author  ：Pancheng Niu
@Date    ：2024/7/17 下午6:57 
'''
import numpy as np
import torch
import matplotlib.pyplot as plt
from scipy.interpolate import griddata
from Helmholtz2D_PINN_without_gamma import Helmholtz2D_PINN, Sampler
import csv
import os
...
if __name__ == '__main__':
    ...
    for depth in [3,5,7]:
        for widths in [50,70]:
            ...
            for mode in ['IAW_PINN']:
                ...
                plt.tight_layout()
                plt.savefig(f'./pic/{mode}_{layer_str}_Exact_Predicted_Absolute_error.pdf',
                ...
                plt.legend()
                plt.tight_layout()
                plt.savefig(f'./pic/{mode}_{layer_str}_lam_log.pdf', dpi=300,
                            bbox_inches='tight')
                plt.close(fig_3)
                # plt.show()
            print("****************************************************************************************")
    ...
    csv_file = os.path.join(save_dir, 'relative_errors_without_gamma.csv')
