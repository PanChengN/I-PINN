...
@File    ：Helmholtz2D_PINN_without_gamma.py
@IDE     ：PyCharm 
@Author  ：Pancheng Niu
@Date    ：2024/7/17 下午6:54 
'''
from DNN import Net_attention, Net
...
class Helmholtz2D_PINN:
    ...
    def AW_loss_fun(self, loss_bcs, loss_res):
        loss = 1. / (self.sigma_u ** 2) * loss_bcs + 1. / (self.sigma_r ** 2) * loss_res + torch.log(
            self.sigma_u ** 2 ) + torch.log(self.sigma_r ** 2)
        return loss
        ...
        for it in pbar:
            ...
            if it % 100 == 0:
                pbar.set_postfix({'Iter': self.iter,
                                  ...
                                  'lam_r': '{0:.2f}'.format(1. / (self.sigma_r ** 2).item()),
                                  'lam_u': '{0:.2f}'.format(1. / (self.sigma_u ** 2).item())
                                  })
            self.iter += 1
