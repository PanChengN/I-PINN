...
@IDE     ：PyCharm 
@Author  ：Pancheng Niu
@Date    ：2024/7/17 下午7:37 
'''
import numpy as np
import torch
import matplotlib.pyplot as plt
from scipy.interpolate import griddata
from Helmholtz2D_PINN_K import Helmholtz2D_PINN, Sampler
import csv
import os
...
if __name__ == '__main__':
    ...
    for lam_threshold in lam_threshold_list:
        for depth in [7]:
            for widths in [50]:
                ...
                for mode in ['PINN', 'IA_PINN', 'IAW_PINN', 'I_PINN']:
                    ...
                    plt.tight_layout()
                    plt.savefig(f'./pic_k_{k}_w_{lam_threshold}/{mode}_{layer_str}_Exact_Predicted_Absolute_error.pdf', dpi=300, bbox_inches='tight')
                    plt.close(fig_1)
                    # plt.show()
                    ...
                    plt.legend()
                    plt.tight_layout()
                    plt.savefig(f'./pic_k_{k}_w_{lam_threshold}/{mode}_{layer_str}_loss.pdf', dpi=300, bbox_inches='tight')
                    plt.close(fig_2)
                    # plt.show()
                    ...
                    plt.legend()
                    plt.tight_layout()
                    plt.savefig(f'./pic_k_{k}_w_{lam_threshold}/{mode}_{layer_str}_lam_log.pdf', dpi=300, bbox_inches='tight')
                    plt.close(fig_3)
                    # plt.show()
                print(
                    "***************************************************************************************************************")
        ...
        save_dir = f'./pic_k_{k}_w_{lam_threshold}/'
        # Relative error folder path
        csv_file = os.path.join(save_dir, 'relative_errors100.csv')
        with open(csv_file, 'w', newline='') as csvfile:
            fieldnames = ['model_type', 'network_structure', 'relative_error','k']
